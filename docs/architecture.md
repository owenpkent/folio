...
| Combine | `src/features/combine/` | Merges two or more PDFs into one document via pdf-lib: pages, metadata, and (best-effort) AcroForm fields carried forward from the inputs, with per-document staging, progress, and cancellation in `store` |
