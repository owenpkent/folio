...
- **`write_document(path, contents)`.** Write the exported PDF to disk: Save-in-place hands it the opened file's own path, Save-a-copy the path the user chose in the native save dialog. The write is atomic (a randomly named temp file in the destination directory, renamed over the target), so a crash or full disk mid-write never leaves a truncated PDF where the user's document was. Living on the Rust side (it mirrors `read_document`) means the frontend needs **no** broad filesystem capability; the previous `fs:allow-write-file` (`$HOME/**`) scope was removed. Rejects non-`.pdf` paths.
