import { PDFName, PDFNumber, PDFString, type PDFDocument, type PDFPage } from 'pdf-lib';
...
function buildHighlight(pdf: PDFDocument, page: PDFPage, annotation: Annotation) {
  ...
  for (const rect of annotation.rects) {
    const { x1, y1, x2, y2 } = toPdfRect(rect, pw, ph);
    quads.push(x1, y2, x2, y2, x1, y1, x2, y1);
  }

  const box = boundingBox(annotation.rects, pw, ph);
...
function buildNote(pdf: PDFDocument, page: PDFPage, annotation: Annotation) {
  ...
  const dict = pdf.context.obj({
    ...
    Contents: PDFString.of(annotation.note?.trim() || annotation.text || ''),
    C: rgbTriplet(annotation.color),
    F: 4,
...
function boundingBox(rects: NormalizedRect[], pw: number, ph: number) {
  const boxes = rects.map((r) => toPdfRect(r, pw, ph));
  return {
