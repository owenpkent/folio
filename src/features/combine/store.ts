import { create } from 'zustand';

import { countPdfPages } from './combineDocuments';

function uid(): string {
  try {
    return crypto.randomUUID();
  } catch {
    return `combine-${Date.now().toString(36)}-${Math.floor(Math.random() * 1e6).toString(36)}`;
  }
}

/** A file staged for the next combine run, in the order it will be merged. */
export interface PendingFile {
  id: string;
  name: string;
  bytes: Uint8Array;
  /** Undefined while the page count is still being read. */
  pageCount?: number;
  /** Set when the file could not be read (corrupt, encrypted, not a PDF). */
  error?: string;
}

/** A file handed to {@link CombineState.open} or {@link CombineState.addFiles}. */
export interface CombineFileSeed {
  name: string;
  bytes: Uint8Array;
}

interface CombineState {
  modalOpen: boolean;
  files: PendingFile[];
  busy: boolean;
  error: string | null;

  /** Open the modal, optionally pre-loaded with files (e.g. from a drop). */
  open(seed?: CombineFileSeed[]): void;
  close(): void;
  addFiles(files: CombineFileSeed[]): void;
  removeFile(id: string): void;
  moveUp(id: string): void;
  moveDown(id: string): void;
  clear(): void;
  setBusy(busy: boolean): void;
  setError(message: string | null): void;
}

/**
 * Transient state for the "Combine PDFs" modal. Nothing here is persisted:
 * the pending file list only exists for the length of one combine run, like
 * the print and OCR progress stores.
 */
export const useCombineStore = create<CombineState>((set, get) => ({
  modalOpen: false,
  files: [],
  busy: false,
  error: null,

  open: (seed) => {
    set({ modalOpen: true, files: [], busy: false, error: null });
    if (seed && seed.length > 0) get().addFiles(seed);
  },

  close: () => set({ modalOpen: false, files: [], busy: false, error: null }),

  addFiles: (newFiles) => {
    const entries: PendingFile[] = newFiles.map((f) => ({ id: uid(), name: f.name, bytes: f.bytes }));
    set((s) => ({ files: [...s.files, ...entries], error: null }));

    // Page counts are read asynchronously and patched onto each entry as they
    // resolve, so a slow or huge file does not hold up the others -- or the
    // dialog's own file-picked resolution -- from showing up in the list.
    for (const entry of entries) {
      void countPdfPages(entry.bytes, entry.name)
        .then((pageCount) => {
          set((s) => ({
            files: s.files.map((f) => (f.id === entry.id ? { ...f, pageCount } : f)),
          }));
        })
        .catch((error: unknown) => {
          const message = error instanceof Error ? error.message : 'Could not read this file';
          set((s) => ({
            files: s.files.map((f) => (f.id === entry.id ? { ...f, error: message } : f)),
          }));
        });
    }
  },

  removeFile: (id) => set((s) => ({ files: s.files.filter((f) => f.id !== id) })),

  moveUp: (id) =>
    set((s) => {
      const idx = s.files.findIndex((f) => f.id === id);
      if (idx <= 0) return s;
      const files = [...s.files];
      [files[idx - 1], files[idx]] = [files[idx], files[idx - 1]];
      return { files };
    }),

  moveDown: (id) =>
    set((s) => {
      const idx = s.files.findIndex((f) => f.id === id);
      if (idx === -1 || idx >= s.files.length - 1) return s;
      const files = [...s.files];
      [files[idx + 1], files[idx]] = [files[idx], files[idx + 1]];
      return { files };
    }),

  clear: () => set({ files: [], error: null }),

  setBusy: (busy) => set({ busy }),
  setError: (error) => set({ error }),
}));
